...
export const compareTo = (currencyItem, currencyGlobal) => {
  ...
  for (let i = 0; i <= currencyItem.prices.length; i++) {
    if (currencyItem.prices[i].currency.symbol === currencyGlobal) {
      currencyData.push(currencyItem.prices[i].currency.symbol, currencyItem.prices[i].amount.toFixed(2));
      return currencyData;
    } else {
      continue;
    }
...
сравниваются selected продуктов из корзины и продукта отправляемого в корзину,
если находится совпадение, то в массив кидаем 1, в обратном случае null */
...
/*функция, которая будет выбирать первые значения аттрибутов продукта, отправленного с PLP,
либо же отправлять продукт у которого нет аттрибутов*/
